...
function insert_cmdi_header(corpus_or_session){
...
function create_cmdi_session(session){
...
function insert_header(MdCreator,MdCreationDate,MdProfile){
...
function insert_cmdi_session_data(session){
...
function insert_cmdi_written_resource(link,size){
...
function insert_cmdi_mediafile(link,size){
...
function insert_cmdi_actor(session,actor_id){
...
var cmdi_generator = function(){
	...
	this.corpus = create_cmdi_corpus();
	this.sessions = [];        
	
	for (var s=0;s<sessions.length;s++){
		this.sessions.push(create_cmdi_session(sessions[s].id));
	}
